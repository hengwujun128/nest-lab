...
export class ExtractionService {
  ...
  private readonly structuredLlm?: Runnable<BaseLanguageModelInput, KgExtractionLlmOutput>
  ...
  constructor(config: ConfigService) {
    ...
    const model = config.get<string>('MODEL_NAME') || config.get<string>('LLM_MODEL') || 'qwen-plus'
   ...
   */
  async extract(content: string, heading: string | null | undefined, documentTitle: string): Promise<ExtractionResult> {
    // 如果内容为空，则返回空结果,省一次无效调用。
    if (!content?.trim()) {
  ...
  private async extractByLlm(
    ...
    if (!this.structuredLlm) {
      throw new Error('KG 抽取未配置 API Key（OPENAI_API_KEY / LLM_API_KEY / DASHSCOPE_API_KEY）')
    }
    ...
    const parsed = await this.structuredLlm.invoke([new SystemMessage(system), new HumanMessage(user)])
    this.logger.log(
  ...
  private toExtractionResult(parsed: KgExtractionLlmOutput): ExtractionResult {
    ...
    for (const e of (parsed.entities ?? []).slice(0, this.maxEntities)) {
      ...
      entities.push({
        ...
        type: normalizeEntityType(e.type), // 规范化实体类型
    ...
    for (const r of (parsed.relations ?? []).slice(0, this.maxRelations)) {
      ...
      relations.push({
        source,
        target,
        relation: normalizeRelationType(r.relation ?? r.type), // 规范化关系类型
