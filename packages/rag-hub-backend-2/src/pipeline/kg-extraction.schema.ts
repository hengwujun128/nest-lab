...
export const kgExtractedEntitySchema = z.object({
  ...
  // Structured Outputs 要求字段全部 required，可空用 nullable 而非 optional
  type: z.string().nullable().describe('实体类型，未知时为 null'),
  description: z.string().nullable().describe('简短描述，可空'),
  aliases: z.array(z.string()).nullable().describe('别名，无则 null 或空数组'),
})
...
export const kgExtractedRelationSchema = z.object({
  ...
  relation: z.string().nullable().describe('关系类型，字段名必须是 relation'),
  type: z.string().nullable().describe('兼容：模型误把关系类型写成 type 时读取'),
  weight: z.number().min(0).max(1).nullable().describe('置信度 0-1，未知时为 null'),
})
