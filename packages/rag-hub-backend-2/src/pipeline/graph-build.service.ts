...
import { auth, type Driver, type Session } from 'neo4j-driver-core'
...
export class GraphBuildService implements OnModuleInit, OnModuleDestroy {
  ...
  async buildForDocument(doc: PipelineDocument): Promise<number> {
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        try {
          extracted = await this.extractionService.extract(chunk.content, chunk.heading, doc.title)
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error)
          this.logger.error(`KG 抽取失败，跳过该块：documentId=${doc.id}, chunk=${chunk.chunkIndex}, ${message}`)
      ...
      this.logger.log(`KG 图谱构建完成：documentId=${doc.id}, chunks=${chunks.length}, entities=${totalEntities}`)
  ...
  /**
   * 图谱关键词检索：从实体的属性上匹配关键词, 匹配实体名/描述、文档标题/摘要、块标题/正文。
   * Neo4j 不可用或关键词为空时返回 []。
   */
