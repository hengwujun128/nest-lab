...
export class DocumentService {
  ...
  async findAll(query: QueryDocumentDto) {
    ...
    // 默认排除已软删记录
    const qb = this.em.createQueryBuilder(DocumentEntity, 'doc').where('doc.deleted = :deleted', { deleted: false })
  ...
  async update(id: string, dto: UpdateDocumentDto, actor?: AuthUser) {
    ...
    if (dto.status !== undefined && dto.status !== doc.status) {
      if (dto.status === DocumentStatus.Draft && doc.status === DocumentStatus.Published) {
        ...
        throw new BadRequestException('请使用 publish / archive / save-draft / 审核接口变更文档状态')
      }
    }
    ...
    if (dto.status !== undefined) {
      if (dto.status === DocumentStatus.Published && doc.status !== DocumentStatus.Published) {
    ...
    await this.syncPipelineAfterUpdate(saved, oldStatus, saved.status, contentChanged, finalContent)
  ...
  async uploadAndCreateDocument(file: Express.Multer.File, meta: UploadParseDto = {}, actor?: AuthUser) {
    ...
    this.logger.log(`上传并解析文件：name=${originalFilename}, size=${file.size}, ext=${extension}`)
