 ...
 */
export type ImageUploader = (bytes: Buffer, fileName: string, contentType: string) => Promise<string>
      ...
      try {
        ...
        for (const page of imageResult?.pages ?? []) {
          ...
          for (const image of page.images ?? []) {
            ...
            if ((image.width > 0 && image.width < threshold) || (image.height > 0 && image.height < threshold)) {
              continue
            }
        ...
        if (pageImageUrls.size > 0) {
          logger.log(`PDF 图片提取完成: ${[...pageImageUrls.values()].reduce((n, a) => n + a.length, 0)} 张`)
        }
      } catch (err) {
        // 整批图片提取失败：降级为纯文本，不抛错
