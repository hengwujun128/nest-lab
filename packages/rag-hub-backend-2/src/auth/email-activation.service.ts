 ...
 * @LastEditTime: 2026-09-14 15:27:22
 * @Description:
 * @FilePath: /nest-lab/packages/rag-hub-backend-2/src/auth/email-activation.service.ts
 */
...
export class EmailActivationService {
  ...
  async createToken(userId: string): Promise<string> {
    ...
    // 生成一个 32 字节的随机字符串，作为 token
    const token = randomBytes(32).toString('hex')
    // NOTE: 这里为什么要生成两个 token？
